...
def generate_riddle(difficulty, user_id):
    ...
    # お願い
    適切な難易度に応じたなぞなぞを一問生成してください。まず、答えが１つに絞られるような論理的な問題を作成してください。次に、その問題をブラッシュアップして、ユーザーが考える楽しみを感じられるよう、比喩的でクリエイティブな表現を使用して、簡単に答えが思い浮かばないような工夫をしてください。

    # 目的
    ユーザーになぞなぞで、考えさせる要素を強調し、解くために発想を働かせる楽しさと驚きを与えるため。

    # 情報
    - 今、ユーザーが選んだ難易度は「{difficulty}」です。難易度に応じて1問なぞなぞを作成し、答えも一緒に提供してください。難易度の種類は以下の3通りです。
        - 簡単: 小学生向けの簡単ななぞなぞ。
        - 普通: 中学生向けの少し難しい問題。
        - 難しい: 大人が楽しめる難しい問題。
