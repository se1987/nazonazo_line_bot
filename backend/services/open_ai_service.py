...
def generate_riddle(difficulty, user_id):
    ...
    prompts = [
        ...
        # プロンプト3: 虫食いクイズ
        f"""
        あなたは言葉遊びの達人です。次のステップに従い、答えがひらがな一文字もしくは漢字一文字で表現される共通点パズル問題を作成してください。

        # お願い
        - ユーザーが3〜4つの言葉や熟語に共通する文字を導き出す必要のある問題を作成してください。
        - 問題の答えは必ずひらがな一文字もしくは漢字一文字で表現できるようにしてください。
        - 答えとなる文字は問題文中では〇で表示してください。

        # 目的
        ユーザーが細部に注目し、論理的に推理するプロセスを楽しめるような問題を作成してください。
        ...
        # 出題例
        ## 1.
        - 問題「【小〇　大〇　〇苗　納〇】〇に共通して入る漢字は何？」
        - 答え: 「豆」
        - 解説: 小豆（あずき）、大豆（だいず）、豆苗（とうみょう）、納豆（なっとう）
        ## 2.
        - 問題「【か〇ら〇しゃ】〇に共通して入る文字は何？」
        - 答え: 「ん」
        - 解説: かんらんしゃ

        # ルール:
        - 問題は1〜4つの言葉や熟語を表示し、必ず答えがひらがな一文字もしくは漢字一文字で表現できるようにしてください。
        - 問題は30文字以内で簡潔に表現してください。
        - 答えとなる文字は問題文中では〇で表示してください。
...
def generate_hint(difficulty, user_id):
    ...
    - ヒントは、{answer}という言葉は使わず、{explanation}に沿った内容の一部を示すように生成してください。
