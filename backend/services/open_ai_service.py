...
def generate_riddle(difficulty, user_id):
    ...
    1. テーマや題材を選んでください（例: 食べ物、日常生活、学校など）。
    2. そのテーマに関連する答えを決め、その答えに関連した特徴や言葉遊びを考えてください。
    3. 難易度に応じて、直感的に答えが分かる簡単なものから、少し考えさせる普通のもの、深く考えさせる難しいものを作成してください。
    4. ユーモアや驚きを加えて、楽しさをプラスしてください。
    5. 最後に、ユーザーが答えを入力できるように、答えも一緒に提供してください。

    - 今、ユーザーが選んだ難易度は「{difficulty}」です。難易度に応じて1問なぞなぞを作成し、答えも一緒に提供してください。
    - **ヒントは生成せず、問題とその答えだけを生成してください**。
    ...
    - 答え: 「豆」
    4. 「一本の木に12本の枝があり　52輪の美しい花が咲き　それぞれの花に7個の花びらがあり　花びらには24本の筋がついている　この木な〜んだ？」
    - 答え: 「1年」
    ...
    - 答え: 「はなたれています」
    4. 「【水〇　〇種　〇園　野〇」〇に共通して入る漢字１文字は？」
    - 答え: 「菜」

    ユーモアと驚きを加え、適切な難易度に応じたなぞなぞを生成してください。出題は以下のフォーマットに従ってください：
    - 問題: xxx
    - 答え: yyy
    """
...
def generate_hint(difficulty, user_id):
    ...
    - 最初のヒントは、あまり答えに近づきすぎないが、解答の手がかりになるヒントを生成してください。
    - ヒントを出すたびに、少しずつ正解に近づけるように調整してください。

    ### 難易度の定義
    - 簡単: 直感的に解ける問題。シンプルなヒントを提供。
    - 普通: 少し考える必要がある問題。中間的な難易度のヒントを提供。
    - 難しい: 深く考えさせる問題。難しい問題へのステップごとのヒントを提供。
    ...
    2. 「風邪をひいた子どもとかけて　牧場の家畜とときます　そのこころは？」
       - 初めのヒント: 鼻に関係する
       - 答え: 「はなたれています」
