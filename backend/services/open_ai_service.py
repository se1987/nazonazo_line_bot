...
def generate_riddle(difficulty, user_id):
    ...
    prompts = [
        ...
        # 目的
        ユーザーの熟語の知識を活用し、新たな知識を増やせるようにするため。
        ...
        # プロンプト4: 虫食いクイズ
        f"""
        あなたは言葉遊びの達人です。次のステップに従い、答えがひらがな一文字で表現される共通点パズル問題を作成してください。

        # お願い
        - ユーザーが1単語内に共通するひらがな一文字を導き出す必要のある問題を作成してください。
        - 問題の答えは必ずひらがな一文字で表現できるようにしてください。
        - 答えとなる文字は問題文中では〇で表示してください。

        # 目的
        ユーザーの言葉の知識を活用し、新たな知識を増やせるようにするため。
        ...
        # ルール:
        - 問題は共通するひらがなが2文字以上含まれる1つの単語をひらがなで表示し、必ず答えがひらがな一文字で表現できるようにしてください。
        - 問題は50文字以内で簡潔に表現してください。
